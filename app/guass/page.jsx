...
function Page() {
  ...
  const calculateGaussElimination = () => {
    ...
    for (let i = 0; i < size; i++) {
      ...
      for (let k = i + 1; k < size; k++) {
        ...
        steps.push(`Eliminate element in row ${k + 1}, column ${i + 1}:`);
        steps.push(formatMatrixForLatex(augmentedMatrix));
      }
    }
    ...
    for (let i = size - 1; i >= 0; i--) {
      solution[i] = augmentedMatrix[i][size] / augmentedMatrix[i][i];
      for (let j = i - 1; j >= 0; j--) {
        augmentedMatrix[j][size] -= augmentedMatrix[j][i] * solution[i];
      }
    }

    setResultX(solution.map(x => x.toFixed(6)));
  ...
  return (
    <div>
      <div className='p-7'>
        <h1 className="text-center font-bold text-4xl p-4">Gaussian Elimination</h1>
      </div>
      ...
      <div className='m-4 md:mx-9'>
        <div className="bg-white rounded-lg w-auto max-w-full">
          ...
          <div className='flex flex-col md:flex-row justify-center items-center mb-4 gap-4'>
            <div className="flex items-center gap-2">
              <button onClick={calculateGaussElimination} className="btn border-2 shadow-lg bg-blue-600 text-white">Calculate</button>
            </div>
          </div>
        </div>
