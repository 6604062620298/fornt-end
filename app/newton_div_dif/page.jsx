...
function page() {
    ...
    const calculatela = () => {
        ...
        for (let i = 0; i < n; i++) {
            ...
            for (let j = 0; j < n; j++) {
                if (i !== j) {
                    sum *= (x_value - x_input[j]) / (x_input[i] - x_input[j]);
                }
            }
            L[i] = sum;
            res += L[i] * y_input[i];
    ...
    const func = (xNew, xOld, i, a, y_input) => {
        ...
        if (i === 0) {
            ...
            return (y_input[xNew] - y_input[xOld]) / (a[xNew] - a[xOld]);
        } else {
            return (func(xNew, xOld + 1, i - 1, a, y_input) - func(xNew - 1, xOld, i - 1, a, y_input)) / (a[xNew] - a[xOld]);
        }
    };
    ...
    return (
        <>
            <header className='p-7 flex justify-center'>
                <h1 className='text-4xl font-bold'>Newton's Divided Difference</h1>
            </header>
